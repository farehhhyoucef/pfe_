...
if (!empty($data)) {
    ...
    foreach ($data as $row) {
        ...
        $type_c = $conn->real_escape_string($row['type_c']);
        $refrence = $conn->real_escape_string($row['n_refernce']);
        $categorie= $conn->real_escape_string($row['categorie']);
        // Construct the SQL query to update the demande_compteur table
        $sql = "UPDATE demande_compteur
                SET n_affire = '$n_affire',reference='$refrence',type_c='$type_c',categorie='$categorie', etat_demande = 'pret'
